...
namespace SDLFramework {
	...
	AssetManager::~AssetManager() {
		for (auto tex : mTextures) { //auto = automaticly puts the data for tex to hold the data (can be any data type)
	...
	TTF_Font* AssetManager::GetFont(std::string filename, int size)
	{
		...
		if (mFonts[key] == nullptr) {
			...
			if (mFonts[key] == nullptr) {
				std::cerr << "Unable to laod font " << filename << "! TTF Font: " << TTF_GetError() << std::endl;
			}
		}
		return mFonts[key];
	}
